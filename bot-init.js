...
const { processBotQuestion, processBotAnswer, saveUserAnswerDB } = require("./process-bot-message");
...
async function createBot(botName, TOKEN) {
  ...
  try {
    ...
    bot.on("callback_query", async (msg) => {
      ...
      try {
        if (await botQuestions[questionInd].reported) {
        ...
        // } else {
        //   // Вопросы закончились, нужно сделать запись в БД
        //   // await saveFinishedSurveyDB();
        // }
      } catch (error) {
